...
**SimpleETL** — веб-приложение (Vue 3 + FastAPI) для автоматизированной обработки текстовых документов. Нарезает текст на чанки, отправляет в LLM для генерации структурированного представления (SPR) и упаковывает результат в Markdown, YAML и HTML файлы для RAG-систем.
...
**HTML вывод.** Формат вывода `html` позволяет быстро готовить веб-страницы из документов для нативной загрузки браузерами без необходимости скачивания самих документов.
